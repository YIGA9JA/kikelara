// server.js (FULL - CLEAN + ADMIN AUTH + ORDERS + DELIVERY PRICING + CONTACT + MESSAGES)
...
const GMAIL_APP_PASS = process.env.GMAIL_APP_PASS || "your_app_password";
...
app.put("/admin/delivery-pricing", requireAdmin, (req, res) => {
  ...
  cleaned.updatedAt = new Date().toISOString(); // ✅ always update timestamp

  writeJson(pricingFile, cleaned);
  res.json({ success: true, pricing: cleaned });
});

// Admin: seed capitals (convenience)
app.post("/admin/delivery-pricing/seed", requireAdmin, (req, res) => {
  ...
  seeded.updatedAt = new Date().toISOString(); // ✅ timestamp
  writeJson(pricingFile, seeded);
...
app.delete("/admin/delete/:index", requireAdmin, (req, res) => {
